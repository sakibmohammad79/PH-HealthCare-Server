...
const initPayment = async (paymentData: IPaymentData) => {
  try {
    ...
    throw new appError(httpStatus.BAD_REQUEST, "Payment error accured!");
  }
};
...
const validatePayment = async (payload: any) => {
  try {
    const response = await axios({
      method: "GET",
      url: `${config.ssl.sslValidationApi}?val_id=${payload.val_id}&store_id=
      ${config.ssl.storeId}&store_passwd=${config.ssl.storePass}&format=json`,
    });
